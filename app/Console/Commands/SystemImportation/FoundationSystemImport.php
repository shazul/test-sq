...
class FoundationSystemImport extends AbstractSystemImport
{
    /**
    ...
    {
        $this->signature = 'soprema:import:foundationSystem {file : CSV containing the systems}';

        $this->description = 'Import and create foundation systems';
        ...
        $this->field_array_key_match = [
            ...
            4  => 'system_' . self::FOUNDATION_SYSTEM . '_family',
            6  => 'system_' . self::FOUNDATION_SYSTEM . '_application_method',
            7  => 'system_bridging',
            20 => 'system_' . self::FOUNDATION_SYSTEM . '_high_performance',
            22 => 'system_guaranty',
        ];
    }
    ...
    {
        $this->info('Import foundation system');
    ...
    {
        $benefits = range(1, 3);
        $group_1_range = range(8, 17);
        $accessories = range(18, 19);
        $tests = range(23, 27);
        $install_video = range(28, 29);
        //mise en vedette
        $is_starred = [21];
        ...
        foreach ($raw_csv_data as $data_row) {
            ...
            $filtered_row['is_starred'] = $data_row[21];
